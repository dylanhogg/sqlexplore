...
def llm_error_response(
    ...
    if kind == "missing_key":
        return response(status="error", message=LLM_MISSING_KEY_MESSAGE)
    if kind == "provider":
        return response(status="error", message=LLM_PROVIDER_ERROR_MESSAGE)
    if kind == "invalid_sql":
        ...
        return response(status="error", message=message, generated_sql=generated_sql)
    assert False, kind
