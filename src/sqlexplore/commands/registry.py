...
def build_command_specs(engine: CommandEngine, completion: CommandCompletionCatalog) -> list[CommandSpec]:
    ...
    return [
        ...
        CommandSpec("/tables", USAGE_TABLES, "Show loaded tables.", _bind(engine, cmd_tables)),
        CommandSpec(
            "/use",
            USAGE_USE,
            "Switch active table for helper commands.",
            _bind(engine, cmd_use),
            completion.complete_use,
        ),
        *[
            CommandSpec(
                item.name,
